OldData2NewData =function(Filename,Directory){
# OldData2NewData(Filename,Directory)
# wandelt altes Format in neues Format automatisch um. 
# INPUT:          
# Filename: 				string, name of the *.data file
#
# Directory: 				string of the directory containing the *.data file
#												(default ==  	getwd() )    
# Output
# umgeschriebene Datei
# author: MT 11/2015
#Altes ReadData
  ...
  #                                                         7	-> character -> Colunms containing descriptions
  ...
  # V$FurtherTexts[1:n,1:c]     array or string matrix with row descriptions (longer 
  #														 descriptions, usually corresponding to Names) to be put 
  #															into the file as last column. If FurtherTexts is not available,
...
################################################################################################################
  ...
  # ColumnNames=ColumnNames[which(DataDefined!=7)] #FurtherTexts Namen aus ColumnNames loeschen
 ...
 # print(Names)
  ...
  DescriptionColumn = which(DataDefined==7) #int mit der 7 spalte
...
if(length(indtmp)==1)
 ...
 if(length(DescriptionColumn)>0){
   # durch i aendert sich die Zaehlung von Datadefined, da names schon geloescht wurde
    FurtherTexts = Data[,DescriptionColumn-i]  #Umspeichern in FurtherTexts 
    Data=Data[,-(DescriptionColumn-i)]  #FurtherTexts als Spalte Loeschen     
    result=list(Data=as.matrix(Data),Header=ColumnNames,Key=Key,DataDefined=DataDefined,Names=Names,FurtherTexts=as.matrix(FurtherTexts))
    
    }else{
   result=list(Data=as.matrix(Data),Header=ColumnNames,Key=Key,DataDefined=DataDefined,Names=Names)     
 }

  #result=list(Data=Data,ColumnNames=ColumnNames,KeyName=KeyName,Key=Key,DataDefined=DataDefined,Checksums=checksums)
  

  return (result)
} # Altes ReadData

  res=ReadData(Filename,InDirectory=Directory)
  if(is.null(res$Names)){
    WriteData(FileName=Filename,OutDirectory=Directory,Data=res$Data,Key=res$Key,Header=res$Header)
  }else{
      WriteData(FileName=Filename,OutDirectory=Directory,Data=res$Data,Key=res$Key,Header=res$Header,Names=res$Names)
